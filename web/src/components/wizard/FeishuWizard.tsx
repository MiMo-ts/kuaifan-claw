    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          >
            <CxIconExternalLink className="w-4 h-4" />
            {step.urlLabel}
          </a>
          ...
          {probeResult && (
            <div className={`rounded-lg p-3 text-sm flex items-start gap-2 ${probeResult.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
              {probeResult.success ? (
                <CxIconCheckCircle className="w-4 h-4 text-green-600 shrink-0 mt-0.5" />
              ) : (
                <CxIconAlertCircle className="w-4 h-4 text-red-600 shrink-0 mt-0.5" />
              )}
        ...
        <div className="px-6 py-4 border-t flex gap-3 justify-between">
          ...
          <div className="flex gap-2">
            ...
            >
              {probing && <CxIconLoader className="w-3.5 h-3.5 animate-spin" />}
              验证凭证
            </button>
