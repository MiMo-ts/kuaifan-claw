    ...
    <div className="space-y-6">
      ...
      {installError && (
        <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 space-y-2">
          <div className="font-medium flex items-center gap-2">
            <CxIconXCircle className="w-4 h-4 text-red-500 shrink-0" />
          ...
          {getPluginErrorHint(installError.message) && (
            <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded p-2 text-xs text-amber-800">
              <CxIconAlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5 text-amber-600" />
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <CxIconLoader className="w-8 h-8 text-blue-500 animate-spin" />
            ...
            {plugins.map((plugin) => (
              ...
              >
                <div className="flex items-center mb-2">
                  ...
                  {plugin.installed ? (
                  <CxIconCheckCircle className="w-5 h-5 text-green-500" />
                ) : null}
                </div>
                ...
                {!plugin.installed ? (
                    ...
                    >
                      {installing === plugin.id ? (
                        <>
                          <CxIconLoader className="w-4 h-4 animate-spin mr-2" />
                          安装中...
                        </>
                      ) : (
                        <>
                          <CxIconDownload className="w-4 h-4 mr-2" />
                          安装
                        </>
                      )}
                    </button>
                  ...
                  <div className="space-y-2">
                    ...
                    {plugin.deps_ready ? (
                      <div className="flex items-center gap-1 text-xs text-green-600">
                        <CxIconCheckCircle className="w-3.5 h-3.5 shrink-0" />
                        <span>依赖就绪</span>
                      </div>
                    ) : (
                      <div className="flex items-start gap-1 text-xs text-orange-600">
                        <CxIconAlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                    ...
                    >
                      {reinstalling === plugin.id ? (
                        <>
                          <CxIconLoader className="w-3.5 h-3.5 animate-spin" />
                          重装中...
                        </>
                      ) : (
                        <>
                          <CxIconRefresh className="w-3.5 h-3.5" />
                          重装依赖
                        </>
                      )}
                    </button>
