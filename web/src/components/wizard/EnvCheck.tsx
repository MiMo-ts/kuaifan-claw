...
export default function EnvCheck({ onNext }: Props) {
  ...
  const getIcon = (status: string) => {
    switch (status) {
      case 'success': return <CxIconCheckCircle className="w-5 h-5 text-green-500" />;
      case 'error': return <CxIconXCircle className="w-5 h-5 text-red-500" />;
      case 'warning': return <CxIconAlertCircle className="w-5 h-5 text-yellow-500" />;
      default: return null;
    }
  };
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <CxIconRefresh className="w-8 h-8 text-blue-500 animate-spin" />
        ...
        <>
            ...
            <div className={`rounded-lg border p-4 ${statusBg(progress.status)}`}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-2">
                  <CxIconLoader className={`w-4 h-4 animate-spin ${statusColor(progress.status)}`} />
          ...
          {logs.length > 0 && (
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">
              <div className="font-medium mb-2 flex items-center gap-1">
                <CxIconGitBranch className="w-3.5 h-3.5" />
                安装日志
              </div>
          ...
          <div className="flex flex-wrap gap-2 justify-between items-center pt-4 border-t">
            <div className="flex flex-wrap gap-2">
              ...
              >
                <CxIconRefresh className={`w-4 h-4 mr-2 ${fixing ? 'animate-spin' : ''}`} />
                重新检测
              </button>
              <button
                type="button"
