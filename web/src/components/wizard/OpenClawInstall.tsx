...
export default function OpenClawInstall({ onNext, onPrev }: Props) {
  ...
  const getIcon = (status: string) => {
    switch (status) {
      ...
      case 'running': return <CxIconLoader className="w-5 h-5 text-blue-500 animate-spin" />;
      default: return null;
    }
  };
          ...
          <div className="flex flex-col items-center gap-3">
            {!cnStatus?.fullyReady && (
              <>
                ...
                >
                  <CxIconLoader className="w-4 h-4 animate-spin hidden" />
                  <span>后台安装（推荐）</span>
                </button>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {liveProgress ? (
                  <>
                    <CxIconLoader className={`w-4 h-4 animate-spin ${statusColor(liveProgress.status)}`} />
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 space-y-3">
          ...
          {getNpmErrorHint(error) && (
            <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded p-3 text-sm text-amber-800">
              <CxIconAlertTriangle className="w-4 h-4 shrink-0 mt-0.5 text-amber-600" />
