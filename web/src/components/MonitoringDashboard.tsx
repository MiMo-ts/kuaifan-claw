    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
            <CxIconActivity className="w-5 h-5 text-blue-600" />
            全模型实时监控
          </h2>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <CxIconRefresh className={`w-5 h-5 ${loading ? 'animate-spin' : ''}`} />
          </button>
        </div>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
          <OverviewCard
            icon={<CxIconActivity className="w-5 h-5" />}
            label="总请求数"
            value={fmtNum(summary.total_requests)}
            color="blue"
          />
          <OverviewCard
            icon={<CxIconTrendingUp className="w-5 h-5" />}
            label="总 Token"
            value={fmtNum(summary.total_tokens)}
            color="indigo"
          />
          <OverviewCard
            icon={<CxIconDollarSign className="w-5 h-5" />}
            label="总费用"
            value={fmtCost(summary.total_cost)}
            color="green"
          />
          <OverviewCard
            icon={<CxIconClock className="w-5 h-5" />}
            label="平均响应"
            value={fmtMs(summary.avg_response_time_ms)}
            color="orange"
          />
          <OverviewCard
            icon={<CxIconZap className="w-5 h-5" />}
            label="活跃供应商"
      ...
      <div className="flex items-center gap-4">
        <CxIconFilter className="w-4 h-4 text-gray-400" />
        <select
      ...
      {modelStats.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
            <CxIconTrendingUp className="w-5 h-5 text-indigo-600" />
            模型用量排行
          </h3>
                ...
                <div className="w-20 text-right">
                  {stat.success_rate >= 99 ? (
                    ...
                    <span className="inline-flex items-center gap-1 text-red-600 text-sm">
                      <CxIconXCircle className="w-4 h-4" />
                      {stat.success_rate.toFixed(1)}%
                    </span>
                  )}
                </div>
      ...
      {filteredMetrics.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
            <CxIconEye className="w-5 h-5 text-green-600" />
            实时指标详情
          </h3>
      ...
      {metrics.length === 0 && !loading && (
        <div className="bg-gray-50 rounded-xl p-12 text-center">
          <CxIconActivity className="w-12 h-12 text-gray-300 mx-auto mb-4" />
