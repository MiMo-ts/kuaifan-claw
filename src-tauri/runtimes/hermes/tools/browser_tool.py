...
from tools.browser_presentation import (
    BACKGROUND_MODE,
    INTERACTIVE_MODE,
    interactive_session_key,
    is_presentation_session_key,
    normalize_mode,
    owner_task_id,
)
...
def _navigation_session_key(task_id: str, url: str, mode: str = BACKGROUND_MODE) -> str:
    """Pick the session key that should handle ``url`` for ``task_id``.

    ``interactive`` always uses a dedicated local session so a desktop user can
    see and operate the bundled Chromium window. ``background`` preserves the
    existing cloud/CDP/local routing behavior.
    ...
    if mode == INTERACTIVE_MODE:
        return interactive_session_key(task_id)
    if _get_cdp_override():
        return task_id
    if _is_camofox_mode():
        return task_id
...
def _is_interactive_session_key(session_key: str) -> bool:
    return is_presentation_session_key(session_key) and session_key.endswith("::interactive")


def _is_forced_local_session_key(session_key: str) -> bool:
    return _is_local_sidecar_key(session_key) or _is_interactive_session_key(session_key)
...
def _bare_task_id_for_session_key(session_key: str) -> str:
    ...
    return owner_task_id(session_key)
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "mode": {
                    ...
                    "default": "background",
                    "description": "Use interactive only for login, typing, clicking, editing, submitting, online documents, online tables, or when the user explicitly asks to open a browser. Use background for research, search, extraction, crawling, screenshots, and summaries."
                }
            },
            "required": ["url"]
        }
...
def _get_session_info(task_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Hybrid routing and Kuaifanclaw interactive sessions force local
    # Chromium regardless of the globally-configured cloud provider. Public
    # background URLs continue to use the original upstream route.
    force_local = _is_forced_local_session_key(task_id)
    ...
    with _cleanup_lock:
        ...
        if _is_interactive_session_key(task_id):
            session_info.setdefault("browser_mode", INTERACTIVE_MODE)
...
def _local_browser_backend_args(session_info: Dict[str, Any]) -> List[str]:
    backend_args = ["--session", session_info["session_name"]]
    if session_info.get("browser_mode") == INTERACTIVE_MODE:
...
def _run_browser_command(
    ...
    # Local mode: --session <name> launches a local Chromium session.
    ...
    if session_info.get("cdp_url"):
        ...
        # Local mode — use the dedicated task session.
        backend_args = _local_browser_backend_args(session_info)
...
def browser_navigate(
    ...
    Args:
        ...
        mode: ``interactive`` opens a visible bundled Chromium session;
            ``background`` preserves the current headless/cloud route.
    ...
    nav_session_key = _navigation_session_key(effective_task_id, url, mode)
    local_session_this_nav = _is_forced_local_session_key(nav_session_key)

    sensitive_query_key = _sensitive_query_param_name(url)
    if sensitive_query_key and not _is_local_backend() and not local_session_this_nav:
        return json.dumps({
            "success": False,
            "error": (
    ...
    if (
        ...
        and not local_session_this_nav
    ...
    if _is_local_sidecar_key(nav_session_key):
        logger.info(
...
def cleanup_browser(
    ...
    *,
    ...
    When ``task_id`` is a bare task identifier, reaps the primary session plus
    any hybrid-routing local sidecar and Kuaifanclaw interactive browser session
    owned by that task. A derived key reaps only its own session. Per-turn
    cleanup can preserve the interactive window so users can continue a
    visible, logged-in workflow after the agent's response is complete.

    Args:
        task_id: Task identifier (or explicit session key)
        preserve_interactive: Keep Kuaifanclaw's visible interactive browser
            session alive. Explicit cleanup, shutdown, and inactivity cleanup
            retain the default ``False`` and still close it.
    """
    if task_id is None:
        task_id = "default"

    # Expand to the full set of session keys to reap. For a bare task_id this
    # includes the primary session plus any local or interactive sidecars.
    if _is_forced_local_session_key(task_id):
        session_keys = [] if preserve_interactive and _is_interactive_session_key(task_id) else [task_id]
        bare_task_id = _bare_task_id_for_session_key(task_id)
    else:
        session_keys = [task_id]
        derived_keys = [
            f"{task_id}{_LOCAL_SUFFIX}",
            interactive_session_key(task_id),
        ]
        with _cleanup_lock:
            session_keys.extend(
                session_key
                for session_key in derived_keys
                if session_key in _active_sessions
                and not (preserve_interactive and _is_interactive_session_key(session_key))
            )
    ...
    # cleaning a derived session drops the binding only if it was still active.
    if _is_forced_local_session_key(task_id):
        if _last_active_session_key.get(bare_task_id) == task_id:
            if not preserve_interactive or not _is_interactive_session_key(task_id):
                _last_active_session_key.pop(bare_task_id, None)
    else:
        active_key = _last_active_session_key.get(bare_task_id)
        if not (preserve_interactive and _is_interactive_session_key(active_key or "")):
            _last_active_session_key.pop(bare_task_id, None)
...
def _chromium_search_roots() -> List[str]:
    ...
    # Bundled offline Chromium lives under <HERMES_HOME>/ms-playwright/.
    # The Rust launcher (commands/runtime.rs) sets PLAYWRIGHT_BROWSERS_PATH
    # to this path, but tests / direct CLI invocations may not inherit
    # that env var. Probe HERMES_HOME explicitly as a fallback so the
    # shipped browser bundle is always discoverable.
    hermes_home = os.environ.get("HERMES_HOME", "").strip()
    if hermes_home:
        bundled = os.path.join(hermes_home, "ms-playwright")
...
registry.register(
    ...
    handler=lambda args, **kw: browser_navigate(
        ...
        mode=args.get("mode", BACKGROUND_MODE),
    ),
